...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [overviewRes, topRes, revenueRes, ordersRes, stockRes] =
          await Promise.all([
        ...
        if (revenueRes.success) {
          ...
          for (let i = 29; i >= 0; i--) {
            ...
            const existing = revenueRes.data.find(
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "pending":
        return (
          <span className="admin-badge admin-badge-warning">Chờ xác nhận</span>
        );
      case "confirmed":
        return (
          <span className="admin-badge admin-badge-info">Đã xác nhận</span>
        );
      case "processing":
        return (
          <span className="admin-badge admin-badge-primary">Đang xử lý</span>
        );
      case "shipping":
        return (
          <span className="admin-badge admin-badge-primary">Đang giao</span>
        );
      case "delivered":
        return <span className="admin-badge admin-badge-success">Đã giao</span>;
      case "cancelled":
        return <span className="admin-badge admin-badge-danger">Đã hủy</span>;
      case "refunded":
        return (
          <span className="admin-badge admin-badge-secondary">Hoàn tiền</span>
        );
      case "returned":
        return (
          <span className="admin-badge admin-badge-secondary bg-dark">
            Trả hàng
          </span>
        );
      default:
        return (
          <span className="admin-badge admin-badge-secondary">{status}</span>
        );
    }
  };
          ...
          <div className="admin-card h-100">
            ...
            <div className="card-body p-0">
              <div
                ...
                style={{ boxShadow: "none", borderRadius: "0 0 16px 16px" }}
              >
                    ...
                    {lowStock.length > 0 ? (
                      lowStock.map((item, index) => (
                        <tr key={item._id || index}>
                          <td>
                            <code className="small text-danger bg-light px-2 py-1 rounded">
                              {item.variant_sku || item.product_id}
                            </code>
                          </td>
                          <td>
                            <span className="admin-badge admin-badge-danger fs-6">
                              {item.quantity}
                            </span>
                          </td>
                        </tr>
